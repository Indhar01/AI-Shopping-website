from flask import Flask, render_template, redirect, flash, url_for, session, logging, request, jsonify,render_template_string
...
'''
    ============================================
            LOGIN & REGISTER ACCESS CODE STARTS
    ============================================
'''
...
'''
    ...
    ============================================
''' 
@app.route('/shared/login.html', methods=['GET','POST'])
def shared_login():
        ...
        if result > 0:
            ...
            if sha256_crypt.verify(str(password_candidate), password):
                ...
                if user_type=="admin":
                    ...
                    flash('Login successful!', 'success')
                    return redirect(url_for('/shopownerprofile.html'))
            
            
            
            else:
                error = 'Invalid email or password! Please try again'
                return render_template('shared/login.html', error=error)
            # Close connection
            cur.close()
        else:
...
def customerprofile():
    cur = mysql.connection.cursor()
    name = session['name']
    query = "SELECT COUNT(*) FROM orders WHERE customer LIKE %s"
    cur.execute(query, ('%' + name + '%',))
    count = cur.fetchone()
    cur.close()

    return render_template('customer/profile.html',count=count)

@app.route('/customer/orders')
def customer_orders():
    cur = mysql.connection.cursor()
    name = session['name']
    query = "SELECT * FROM orders WHERE customer LIKE %s"
    cur.execute(query, ('%' + name + '%',))
    orders = cur.fetchall()
    cur.close()
...
'''
    ============================================
            SHOP-OWNER PANEL ROUTES STARTS
    ============================================
''' 

@app.route('/shopownerprofile.html')
@is_logged_in
def shopownerprofile():
    return render_template('shopowner/shopowner_profile.html')

'''
    ============================================
            SHOP-OWNER PANEL ROUTES ENDS
    ============================================
''' 
...
def add_product():
    ...
    if request.method == 'POST' and form.validate():
        ...
        # Execute query
        cur.execute("INSERT INTO products(article_no,prd_name, prd_description, prd_type, prd_category ,prd_price,prd_min_price, image_path) VALUES(%s, %s, %s, %s, %s, %s, %s, %s)",
...
'''
    ============================================
            LOGIN & REGISTER ACCESS CODE ENDS
    ============================================
'''
...
@app.route('/shop/getdpproducts',methods=['GET','POST'])
def disc_page():
    article_no = request.args.get('prd')
    session['prd'] = article_no
    cur = mysql.connection.cursor()
    ...
    #recommendations = recommend(article_no, num=8)
    #app.logger.info(recommendations)
    return render_template('Discount_page.html', product=product, products=get_all_products())
...
def user_offers(user_offer=0):
    if 'cart' in session:
        pass
    return user_offer

@app.route('/cart')
def display_cart(data=0):
    if 'cart' in session:
        # app.logger.info(prods)
        # app.logger.info(len(prods))
        cur = mysql.connection.cursor()
    ...
    #app.logger.info('cart' in session)
        ...
        return render_template('cart.html', products=products,data=data)
...
def add_to_cart():
    ...
    if 'discount_price' in session:
        ...
        disc_price = user_offers()

    print(disc_price)
    #disc_price=session['cart'][article_no][-1]
    if size_selected and quantity and size_selected != 'nosize' and quantity != "0" and disc_price==0 :
        ...
        if 'cart' in session:
            ...
            print(session['cart'])
            return jsonify({'success': 'Product added to cart!'})
    elif size_selected and quantity and size_selected != 'nosize' and quantity != "0" and disc_price>0 :
        if not any(article_no in d for d in session['cart']):
                #app.logger.info('adding a product in cart')
                session['cart'].update({article_no: [quantity, size_selected, disc_price]})
                print(session['cart'])
...
def extract_amount(sentence): 
    ...
    try:
        ...
        import re
        match = re.search(r"rs\.(\d+)/?-?", sentence)
            ...
            for i in intents['intents']:
                ...
                if i['tag'] == results[0][0]:
                    ...
                    if not 'context_filter' in i or \
                        ...
                        if i['tag'] == 'customeroffers' or ('rs.' in sentence):
...
def get_bot_response():
    ...
    try: 
        ...
        elif res == 'Thank you for your time. You can now add the item to cart to avail discounted price' :
        ...
        elif res =='Thanks! we have a deal. Please click on add to cart button now':
        ...
        elif res =='Thank you. Click on add to cart button to avail the discounted price':
        ...
        print('error')
        #print(session['cart'].update({article_no: [5,'small',1000]}))
        res = response("bla bla", '123',min_acceptable_price, user_offer, original_price)
...
def thankyou():
    try:
        ...
        phno=request.form['c_phone']
        total=request.form['total']
        quantity=request.form['quantity']
        # address=request.form["c_address"]
        address=request.form["c_address"]+' '+request.form["c_address1"]+' '+request.form['c_state_country']+' '+request.form['c_postal_zip']
        # print(email)
        # print(phno)
        # print(total)
        # print(quantity)
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO orders (order_status,order_amount,Quantity,customer, Email, Phno, address) VALUES (%s,%s,%s,%s, %s, %s, %s)",('Placed',total,quantity,customer,email,phno,address))
        mysql.connection.commit()
        cur.close()
        session['cart'].clear()
...
if __name__ == '__main__':
    app.secret_key = 'indhar'
    app.run(debug=True, port=5050,host=socket.gethostbyname(socket.gethostname()))
